...
public class XrdService {
    ...
    public XrdData analyzeXrdFileFromStream(InputStream inputStream) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream))) {
            ...
            logger.error("Error parsing XRD file from stream", e);
            throw e;
        }

        if (angles.isEmpty() || intensities.isEmpty()) {
            logger.error("No valid data found in XRD stream");
            throw new IllegalArgumentException("No valid XRD data found in stream");
        }
    ...
    public XrdData analyzeXrdFile(MultipartFile file) throws Exception {
        ...
        return analyzeXrdFileFromStream(file.getInputStream());
    }
    ...
    // if (angles.isEmpty() || intensities.isEmpty()) {
    // logger.error("No valid data found in XRD file: {}",
